...
class ReportEngine(ss: SparkSession, am: Map[ String, Any ], an: String, sp: String ) {
  ...
  /**
    ...
    * Also allows for fetching credentials from 1) properties files or 2) yarn environment vars (default)
    */
  private def loadCohortsAndMeasures(): Unit = {
    ...
    if ( useCachedCohorts ) {
      ...
      val cohorts = spark.read.format( "csv" ).option( "header", "true" ).load( cohortsS3 ).cache()
      val measures = spark.read.format( "csv" ).option( "header", "true" ).load( measuresS3 ).cache()

      cacheCohortsAndMeasures( cohorts, measures )

    } else {

      // we're getting data live from JDBC source...
      val ( jdbcUsername: String, jdbcPassword: String, jdbcUrl: String ) = {

        // ...now what about credentials: Defaults to using environment, NOT properties file
        if ( argsMap.getOrElse( "credentials-source", "" ) == "properties" ) {
          getCredentialsFromProperties()
        } else {
          // defaults to getting from environment
          getCredentialsFromYarnEnvironment()
        }
      }
  ...
  /**
    * Helper for loadCohortsAndMeasures
    * @param cohorts
    * @param measures
    */
  private def cacheCohortsAndMeasures( cohorts: DataFrame, measures: DataFrame ): Unit = {
  ...
  /**
    * Helper method for loadCohortsAndMeasures
    * @return ( jdbcUsername, jdbcPassword, jdbcUrl )
    */
  private def getCredentialsFromProperties(): ( String, String, String ) = {
    ...
    var jdbcUrl = s"jdbc:mysql://${jdbcHostname}:${jdbcPort}/${jdbcDatabase}?user=${jdbcUsername}&password=${jdbcPassword}"

    if ( debugging ) {

      println("\n\n=======================================================================")
      println(s"PROPERTIES: jdbcUrl      [$jdbcUrl]")
      println(s"PROPERTIES: jdbcUsername [$jdbcUsername]")
      println(s"PROPERTIES: jdbcPassword [$jdbcPassword]")
      println("=======================================================================")
    }

    ( jdbcUsername, jdbcPassword, jdbcUrl )
  }
  /**
    * Helper method for loadCohortsAndMeasures
    * @return ( jdbcUsername, jdbcPassword, jdbcUrl )
    */
  private def getCredentialsFromYarnEnvironment(): ( String, String, String ) = {

    var jdbcUrl      = sys.env.getOrElse( "RDS_URL",      "NOT FOUND" )
    var jdbcUsername = sys.env.getOrElse( "RDS_USERNAME", "NOT FOUND" )
    var jdbcPassword = sys.env.getOrElse( "RDS_PASSWORD", "NOT FOUND" )

    jdbcUsername = getPlainText( jdbcUsername, Collections.singletonMap( "Name", "RDSUser" ) )
    jdbcPassword = getPlainText( jdbcPassword, Collections.singletonMap( "Name", "RDSPassword" ) )

    // build url
    jdbcUrl = s"$jdbcUrl&user=${jdbcUsername}&password=${jdbcPassword}"

    if ( debugging ) {

      println("\n\n=======================================================================")
      println(s"YARN/ENV: jdbcUrl      [$jdbcUrl]")
      println(s"YARN/ENV: jdbcUsername [$jdbcUsername]")
      println(s"YARN/ENV: jdbcPassword [$jdbcPassword]")
      println("=======================================================================")
    }
    ( jdbcUsername, jdbcPassword, jdbcUrl )
  }
  ...
  private def loadAndPrepareInput(): Unit = {
    ...
    val inputRawDf = spark.read.parquet( inputPath )
    ...
    var inputRawReducedRenamedDf = inputRawReducedDf.toDF( updatedColNames: _* )
    ...
    for ( dateColumnName <- dateColumnNamesList ) {
      ...
      // TODO: Remove when we're certain that datasets have all required columns
    ...
    // we've got ONE column that needs to be recast as float
    // TODO: make this dynamic, not hard coded
    inputRawReducedRenamedDf = inputRawReducedRenamedDf.withColumn( "p1704", inputRawReducedRenamedDf( "p1704" ).cast( "float" ) )
    ...
    if ( argsMap.getOrElse( "load-subsets-dynamically", "true" ) == "true" ) {
      ...
      subsetsList = spark.sql( s"SELECT DISTINCT $colName FROM input WHERE LENGTH( $colName ) < 3" ).rdd.map( row => row( 0 ).asInstanceOf[ String ] ).collect.sorted
      printBanner( s"subsetsList [${subsetsList.mkString( "," )}]" )
    }
    ...
    if ( debugging ) {
      ...
      println( s"$STATS_PREFIX Time to load and whittle down production input records from file: [$seconds]s" )
    }
  }
  /**
  ...
  def startInteractiveMode(): Unit = {
    ...
    if ( debugging ) {
      ...
      println( f"$STATS_PREFIX Initialization of [$appName] complete in [$seconds%.1f] seconds.\n\n" )
    }
      ...
      inputs( 0 ) match {
        ...
        case "r"   => {
          ...
          if ( debugging ) {
            ...
            println( s"$STATS_PREFIX Time to run query( ies ) [$seconds]s -or- [$minutes]m" )
          }
        }
        case "dr"  => {
  ...
  private def getAndRunMultilineSql(): Unit = {
    ...
    if ( debugging ) {
      ...
      println( s"$STATS_PREFIX Time to run query [$millis]ms --or-- [$seconds]s" )
      println( s"$STATS_PREFIX Query [$accumulatedInput]" )
    }
  }
  ...
  private def runMedianCountRule( query: String, id: String ): String = {
    ...
    if ( !dryRun ) {
      ...
      if ( debugging ) {
        ...
        println( s"$STATS_PREFIX Time to sort and calculate median [$millis]ms" )
      }
    }
    // done!
    result.toString
  ...
  private def runMedianSubsetsRule( query: String, id: String ): Unit = {
    ...
    if ( !dryRun ) {
      ...
      if ( debugging ) {
        ...
        println( s"$STATS_PREFIX Time to calculate [${subsetsList.length}] medians: [${seconds}]s mean s/subset [$meanCalcTime]" )
      }
    }
  }
  ...
  private def runCountRule( queryString: String, id: String ): Long = {
    ...
    if ( dryRun ) {
      ...
      if ( debugging ) println( s"$STATS_PREFIX RunCountRule called, time to run [$id]: [${System.currentTimeMillis() - startMillis}]ms" )
    }
    count
  }
  ...
  def stop(): Unit = {
    ...
    if ( debugging ) {
      ...
      println( f"$STATS_PREFIX $appName run complete in [$minutes%.1f] minutes.\n\n" )
    }
  }
  ...
  private def processResults(): Unit = {
    ...
    for ( cellId <- resultsMap.keys.toArray.sorted ) {
      ...
      if ( expected == "" && actual == "" ) {
        ...
        if ( showMissing ) println( s"$STATS_PREFIX Query id-subset: [$id]-[$subset] Expected [MISSING] == Actual: [MISSING] [false]" )

      } else if ( expected == "" ) {

        missing += 1
        comparison = ( id, subset, getNumericOrNothing( actual ), num, den, "MISSING", actual, "false", query )
        if ( showMissing ) println( s"$STATS_PREFIX Query id-subset: [$id]-[$subset] Expected [MISSING] == Actual: [$actual] [false]" )

      } else if ( actual == "" ) {

        missing += 1
        comparison = ( id, subset, getNumericOrNothing( actual ), num, den, expected, "MISSING", "false", query )
        if ( showMissing ) println( s"$STATS_PREFIX Query id-subset: [$id]-[$subset] Expected [$expected] == Actual: [MISSING] [false]" )

      } else  if ( expected == actual ) {

        hits += 1
        comparison = ( id, subset, getNumericOrNothing( actual ), num, den, expected, actual, "true", query )
        if ( showHits ) println( s"$STATS_PREFIX Query id-subset: [$id]-[$subset] Expected [$expected] == Actual: [$actual] [${expected == actual}]" )

      } else {

        misses += 1
        comparison = ( id, subset, getNumericOrNothing( actual ), num, den, expected, actual, "false", query )
        if ( showMisses ) println( s"$STATS_PREFIX Query id-subset: [$id]-[$subset] Expected [$expected] == Actual: [$actual] [${expected == actual}]" )
      }
      comparisons += comparison
    }
    val accuracy = ( hits * 1.0 / ( hits + misses + missing ) ) * 100
    printBanner( f"$STATS_PREFIX Hits: [$hits], misses: [$misses], missing: [$missing], accuracy: [$accuracy%.2f]%%" )
    ...
    if ( debugging ) {
      ...
      println( s"$STATS_PREFIX Total time spent calculating medians [$minutes] minutes" )
    }
  }
...
//      }
      ...
      val s3path = getUniqueResultsPath( argsMap.get( "s3-output" ).get.toString + "-" + appName.replace( " ", "-" ).toLowerCase + "-" + windowType + "-q" + quarterID )
      ...
      if ( debugging ) {
        println( s"$STATS_PREFIX Time to write results to S3 [${System.currentTimeMillis() - startMillis}]ms" )
        println( s"$STATS_PREFIX s3path  [$s3path]" )
      }
    }
  }
